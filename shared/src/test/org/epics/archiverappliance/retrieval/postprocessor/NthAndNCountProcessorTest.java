...
public class NthAndNCountProcessorTest {
    ...
    private ArrayListEventStream getData(short year) {
        ...
        ArrayListEventStream testData = new ArrayListEventStream(
                totSamples, new RemotableEventStreamDesc(ArchDBRTypes.DBR_SCALAR_DOUBLE, pvName, year));
        for (int s = 0; s < totSamples; s++) {
            testData.add(new SimulationEvent(
                    startOfSamples.getSecondsintoyear() + s * 60,
                    year,
                    ArchDBRTypes.DBR_SCALAR_DOUBLE,
                    new ScalarValue<>((double) s)));
        }
        ...
        for (int i = 0; i < sampling.length; i++) {
            ...
            for (Event e : retData) {
                ...
                Assertions.assertEquals(
                        testData.get(n).getSampleValue().getValue().doubleValue(),
    ...
    public void testNthProcessorTruncation() throws Exception {
        ...
        YearSecondTimestamp startOfSamples = TimeUtils.convertToYearSecondTimestamp(
                TimeUtils.convertFromISO8601String(year + "-06-01T10:00:00.000Z"));
        ArrayListEventStream testData = new ArrayListEventStream(
                Nth.MAX_COUNT + 1, new RemotableEventStreamDesc(ArchDBRTypes.DBR_SCALAR_DOUBLE, pvName, year));
        for (int s = 0; s < Nth.MAX_COUNT + 1; s++) {
            testData.add(new SimulationEvent(
                    startOfSamples.getSecondsintoyear() + s,
                    year,
                    ArchDBRTypes.DBR_SCALAR_DOUBLE,
                    new ScalarValue<>((double) s)));
        }
        ...
        for (int i = 2; i < 5; i++) {
            ...
            for (Event e : retData) {
                Assertions.assertEquals(
                        ...
                        e.getSampleValue().getValue().intValue(),
                        "The returned value should be equal to the expected number of samples");
                eventCount++;
            }
