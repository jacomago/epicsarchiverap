...
import org.epics.archiverappliance.config.CoreConfigService.CachedPVCounts;
...
public class EngineContext {
    ...
    private final class DisconnectChecker implements Runnable {
        ...
        public void run() {
            try {
                ...
                logger.info(
                        "Disconnected channels {} channels needing to start meta channels {} out of a total of {}",
                        disconnectedChannelCount,
                        needToStartMetaChannelPVNames.size(),
                        totalChannels);
                        ...
                        for (ApplianceInfo applianceInfo : configService.getAppliancesInCluster()) {
                            if (applianceInfo
                                ...
                                try {
                                    ...
                                    // This really needs to be based on some cluster-wide event where all the appliances
                                    // have had their chance at connecting to their PVs.
     ...
     */
    public static class CommandThreadChannel {
    ...
    public int getCAJChannelCount() {
        ...
        for (JCACommandThread commandThread : command_threads) {
            totalCAJChannelCount += commandThread.getTotalChannelCount();
        }
