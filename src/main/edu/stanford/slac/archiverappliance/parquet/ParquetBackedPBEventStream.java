package edu.stanford.slac.archiverappliance.parquet;
...
import static edu.stanford.slac.archiverappliance.parquet.ParquetInfo.NANOSECONDS_COLUMN_NAME;
import static edu.stanford.slac.archiverappliance.parquet.ParquetInfo.SECONDS_COLUMN_NAME;
    ...
    private record TimePeriod(YearSecondTimestamp startYst, YearSecondTimestamp endYst) {
        ...
        FilterCompat.Filter filter() {
            ...
            FilterPredicate predicate = and(
                    ...
                    or(
                            and(
                                    eq(intColumn(SECONDS_COLUMN_NAME), startYst.getSecondsintoyear()),
                                    gtEq(intColumn(NANOSECONDS_COLUMN_NAME), startYst.getNano())),
                            gtEq(intColumn(SECONDS_COLUMN_NAME), startYst.getSecondsintoyear() + 1)),
                    // ltEq end
                    or(
                            lt(intColumn(SECONDS_COLUMN_NAME), endYst.getSecondsintoyear()),
                            and(
                                    eq(intColumn(SECONDS_COLUMN_NAME), endYst.getSecondsintoyear()),
                                    ltEq(intColumn(NANOSECONDS_COLUMN_NAME), endYst.getNano()))));
