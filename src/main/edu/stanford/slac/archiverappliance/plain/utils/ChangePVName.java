...
public class ChangePVName {
    ...
    public static void main(String[] args) throws Exception {
        if (args == null || args.length <= 2) {
            System.out.println(
                    "Usage: run.sh edu.stanford.slac.archiverappliance.PlainPB.utils.ChangePVName <OldPVName> <NewPVName> <PBFiles>");
            return;
        }
        ...
        for (String fileName : files) {
            ...
            if (!info.getPVName().equals(oldPVName)) {
                System.out.println(
                        fileName + " is not associated with PV " + oldPVName + ". Skipping this just to be safe");
                continue;
            }
