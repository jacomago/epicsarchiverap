                    ...
                    for (Event e : strm) {
                        {
                            ...
                            if (statcount == null) {
                                statcount = new Count(status);
                                statuses.put(status, statcount);
                            }
                            statcount.count++;
                        }

                        {
                            ...
                            if (sevrcount == null) {
                                sevrcount = new Count(severity);
                                severities.put(severity, sevrcount);
                            }
                            sevrcount.count++;
                        }
                    }
...

        {
            ...
            for (Count c : sortedStatuses) {
                System.out.println(MessageFormat.format(
                        "{0, number, integer}:\t{1, number, integer}:\t{2, number, percent}",
                        c.val,
                        c.count,
                        (((float) c.count) / total)));
            }
