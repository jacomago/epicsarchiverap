            ...
            try (LineByteStream lis = new LineByteStream(path)) {
                ...
                while (nextLine != null) {
                    if (unmarshallingConstructor == null) {
                        PayloadInfo info = PayloadInfo.parseFrom(
                            LineEscaper.unescapeNewLines(nextLine)
                        );
                        ArchDBRTypes dbrType = ArchDBRTypes.valueOf(
                            info.getType()
                        );
                        year = (short) info.getYear();
                        System.out.println(
                            "Parsing payload info type is " +
                                dbrType +
                                " and data is for year " +
                                year +
                                " for the PV " +
                                info.getPvname() +
                                " Elementcount is " +
                        ...
                        unmarshallingConstructor =
                            DBR2PBTypeMapping.getPBClassFor(
                                dbrType
                            ).getUnmarshallingFromByteArrayConstructor();
                    } else {
                        if (nextLine.length <= 0) {
                            System.out.println(
                                "Resetting the unmarshallingConstructor"
                            );
                            unmarshallingConstructor = null;
                        } else {
                            try {
                                DBRTimeEvent ev =
                                    (DBRTimeEvent) unmarshallingConstructor.newInstance(
                                        year,
                                        new ByteArray(nextLine)
                                    );
                                System.out.println(
                                ...
                                logger.error(
                                    "Exception unmarshalling line: " + lineNum,
                                    ex
                                );
                            }
                        }
