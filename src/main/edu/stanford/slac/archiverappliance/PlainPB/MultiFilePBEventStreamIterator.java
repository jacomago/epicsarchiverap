...
public class MultiFilePBEventStreamIterator implements Iterator<Event> {
    ...
    public MultiFilePBEventStreamIterator(LineByteStreamCreator[] istreams, short year, ArchDBRTypes type)
            throws IOException {
    ...
    public Event next() {
        try {
            return DefaultEvent.fromByteArray(type, year, new ByteArray(nextLine));
        } catch (Exception ex) {
            logger.error("Exception creating event object", ex);
            return null;
        }
