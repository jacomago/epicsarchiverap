...
public class PBFileInfo {
    ...
    public PBFileInfo(Path path, boolean lookupLastEvent) throws IOException {
        try (LineByteStream lis = new LineByteStream(path)) {
            ...
            if (firstLine != null) {
                firstEvent = (DBRTimeEvent) DefaultEvent.fromByteArray(type, getDataYear(), new ByteArray(firstLine));
                if (lookupLastEvent) {
    ...
    private void lookupLastEvent(Path path, LineByteStream lis, ArchDBRTypes archDBRTypes) throws Exception {
        // If we do not have a first line, we probably do not have a last line
        ...
        while (lastEvent == null && lastLine != null && tries < 1000) {
            try {
                lastEvent = DefaultEvent.fromByteArray(archDBRTypes, getDataYear(), new ByteArray(lastLine));
