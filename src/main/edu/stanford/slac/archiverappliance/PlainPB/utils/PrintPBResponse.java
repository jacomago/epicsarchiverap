...
public class PrintPBResponse {
    private static final Logger logger = LogManager.getLogger(PrintPBResponse.class.getName());

    public static void main(String[] args) throws Exception {
        if (args == null || args.length < 1) {
            System.err.println(
                    "Usage: java edu.stanford.slac.archiverappliance.PlainPB.utils.PrintTimes <PB/HTTP response dump>");
            return;
        }
            ...
            try (LineByteStream lis = new LineByteStream(path)) {
                ...
                while (nextLine != null) {
                    if (dbrType == null) {
                        ...
                        System.out.println("Parsing payload info type is " + dbrType + " and data is for year " + year
                                + " for the PV " + info.getPvname() + " Elementcount is " + info.getElementCount());
                        ...
                        if (nextLine.length <= 0) {
                            ...
                            try {
                                DBRTimeEvent ev = (DBRTimeEvent)
                                        DefaultEvent.fromByteArray(dbrType, year, new ByteArray(nextLine));
                                System.out.println(
                                        TimeUtils.convertToISO8601String(((DBRTimeEvent) ev).getEventTimeStamp())
                                                + "\t"
                                                + TimeUtils.convertToHumanReadableString(
                                                        (((DBRTimeEvent) ev).getEventTimeStamp()))
