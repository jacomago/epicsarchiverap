...
public class PBScalarString implements DBRTimeEvent {
	...
	public PBScalarString(short year, ByteArray bar) {
		this.bar = bar;
		this.year = year;
	}

	public PBScalarString(short year, Message.Builder eventBuilder) {
		this.year = year;
		this.dbevent = (EPICSEvent.ScalarString) eventBuilder.build();
		bar = new ByteArray(LineEscaper.escapeNewLines(dbevent.toByteArray()));
	}

	public PBScalarString(DBRTimeEvent ev) {
	...
	public Event makeClone() {
		return new PBScalarString(this);
	}
	...
	public SampleValue getSampleValue() {
		unmarshallEventIfNull();
		return new ScalarStringSampleValue(dbevent.getVal());
	}
	...
	public void setRepeatCount(int repeatCount) {
		unmarshallEventIfNull();
		dbevent = EPICSEvent.ScalarString.newBuilder().mergeFrom(dbevent).setRepeatcount(repeatCount).build();
		bar = new ByteArray(LineEscaper.escapeNewLines(dbevent.toByteArray()));
		return;
	}
	...
	public void setStatus(int status) {
		unmarshallEventIfNull();
		if(status != 0) { 
			dbevent = EPICSEvent.ScalarString.newBuilder().mergeFrom(dbevent).setStatus(status).build();
		} else { 
			dbevent = EPICSEvent.ScalarString.newBuilder().mergeFrom(dbevent).build();
		}
		bar = new ByteArray(LineEscaper.escapeNewLines(dbevent.toByteArray()));
		return;
	}
	...
	public void setSeverity(int severity) {
		unmarshallEventIfNull();
		if(severity != 0) { 
			dbevent = EPICSEvent.ScalarString.newBuilder().mergeFrom(dbevent).setSeverity(severity).build();
		} else { 
			dbevent = EPICSEvent.ScalarString.newBuilder().mergeFrom(dbevent).build();
		}
		bar = new ByteArray(LineEscaper.escapeNewLines(dbevent.toByteArray()));
	...
	private void unmarshallEventIfNull() {
		try {
			if(dbevent == null) {
				dbevent = EPICSEvent.ScalarString.newBuilder().mergeFrom(bar.inPlaceUnescape().unescapedData, bar.off, bar.unescapedLen).build();
			}
		} catch(Exception ex) {
	...
	public void addFieldValue(String fieldName, String fieldValue) {
		...
		dbevent = EPICSEvent.ScalarString.newBuilder().mergeFrom(dbevent).addFieldvalues(fv).build();
		bar = new ByteArray(LineEscaper.escapeNewLines(dbevent.toByteArray()));
		return;
	}
	...
	public void markAsActualChange() {
		unmarshallEventIfNull();
		dbevent = EPICSEvent.ScalarString.newBuilder().mergeFrom(dbevent).setFieldactualchange(true).build();
		bar = new ByteArray(LineEscaper.escapeNewLines(dbevent.toByteArray()));
		return;
	}
	...
	public void setFieldValues(HashMap<String, String> fieldValues, boolean markAsActualChange) {
		...
		dbevent = EPICSEvent.ScalarString.newBuilder().mergeFrom(dbevent).addAllFieldvalues(fieldValuesList).setFieldactualchange(markAsActualChange).build();
		bar = new ByteArray(LineEscaper.escapeNewLines(dbevent.toByteArray()));
		return;
	}	
