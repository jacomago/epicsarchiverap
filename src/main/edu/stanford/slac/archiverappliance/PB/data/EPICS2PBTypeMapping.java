...
public class EPICS2PBTypeMapping {
    ...
    private static final HashMap<ArchDBRTypes, EPICS2PBTypeMapping> typemap =
            new HashMap<ArchDBRTypes, EPICS2PBTypeMapping>();
    ...
    private EPICS2PBTypeMapping(Class<? extends DBRTimeEvent> pbClass) {
        ...
        if (JCADBRConstructor == null && EPICSV4DBRConstructor == null) {
            String msg = "Cannot get a DBR constructor for PB event for class " + pbClass.getName()
                    + ". We should either have something that comes from JCA or from Epics V4";
            logger.error(msg);
