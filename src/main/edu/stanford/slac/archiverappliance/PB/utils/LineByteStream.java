...
public class LineByteStream implements Closeable {
        ...
        if (totalBytesReadSoFar >= totalBytesToRead) {
            ...
            if (resetBytesRead <= bytesRead) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Cannot find newline at tail end of file. resetBytesRead = " + resetBytesRead
                            + " bytesRead=" + bytesRead + " totalBytesReadSoFar=" + totalBytesReadSoFar
                            + "totalBytesToRead=" + totalBytesToRead);
                }
            }
        }
        // We leave totalBytesReadSoFar so far at the higher value so the next readNextBatch will terminate at the first
        // if statement.
    }
    ...
    public byte[] readLine() throws IOException {
        ...
        throw new LineTooLongException(
                "Unable to determine end of line within iteration count " + MAX_ITERATIONS_TO_DETERMINE_LINE);
    }
            ...
            try {
                while (currentReadPosition < bytesRead) {
                    ...
                    if (currentReadPosition >= bytesRead - 1) {
                        readNextBatch();
                        if (bytesRead <= 0 || currentReadPosition >= bytesRead) {
                            // We have not found a new line; we cannot return what we have as we'll get
                            // PBParseExceptions upstream.
                            bar.reset();
                            return bar;
                        }
                    } else {
    ...
    public void seekToBeforeLastLine() throws IOException {
        ...
        while (loopcount < MAX_ITERATIONS_TO_DETERMINE_LINE) {
            ...
            if (seekPos == 0) {
                logger.debug(
                        "Is it possible that the file has only line? We have come to the beginning of the file and this should be definitely before the last line.");
                return;
            }
    ...
    public void seekToBeforePreviousLine(long posn) throws IOException {
        ...
        while (loopcount < MAX_ITERATIONS_TO_DETERMINE_LINE) {
            ...
            // If we are reading the first block, we read more than what we need; so adjust what we read to where we
            // need to be.
            if (posn < bytesRead) {
                bytesRead = (int) posn;
            }
