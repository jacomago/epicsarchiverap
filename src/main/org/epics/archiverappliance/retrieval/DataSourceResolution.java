...
public class DataSourceResolution {
     ...
     */
    public LinkedList<UnitOfRetrieval> resolveDataSources(
            String pvName,
            Instant start,
            Instant end,
            PVTypeInfo typeInfo,
        ...
        if (!applianceForPV.equals(configService.getMyApplianceInfo())) {
            ...
            List<DataSourceforPV> dataSources =
                    RetrievalState.of(configService).getDataSources(context, pvName, typeInfo, start, end, req);
            ...
            if (postProcessor instanceof TimeSpanDependentProcessing) {
                ...
                spannedProcessors =
                        ((TimeSpanDependentProcessing) postProcessor).generateTimeSpanDependentProcessors(yearlySpans);
            } else {
            ...
            for (TimeSpanDependentProcessor spannedProcessor : spannedProcessors) {
                for (DataSourceforPV dataSource : dataSources) {
                    ...
                    try {
                        if (dataSource.isOverridingStartAndEndTimes()
                                && spannedProcessor.getTimeSpan().contains(dataSource.getRequestTimeSpan())) {
                            if (logger.isDebugEnabled()) {
                                logger.debug("Adding data source "
                                        + dataSource.getStoragePlugin().getDescription()
                                        + " using post processor "
                                        + spannedProcessor.getPostProcessor().getIdentity()
                                        + " from start time "
                                        + TimeUtils.convertToHumanReadableString(
                                                dataSource.getRequestTimeSpan().getStartTime())
                                        + " to end time "
                                        + TimeUtils.convertToHumanReadableString(
                                                dataSource.getRequestTimeSpan().getEndTime()));
                            }
                            ...
                            unitsofretrieval.add(new UnitOfRetrieval(
                                    ...
                                    dataSource.getRequestTimeSpan().getStartTime(),
                                    dataSource.getRequestTimeSpan().getEndTime(),
                                    spannedProcessor.getPostProcessor(),
                                    context));
                        } else {
                            if (logger.isDebugEnabled()) {
                                logger.debug("Adding data source "
                                        ...
                                        + " from start time "
                                        + TimeUtils.convertToHumanReadableString(
                                                spannedProcessor.getTimeSpan().getStartTime())
                                        + " to end time "
                                        + TimeUtils.convertToHumanReadableString(
                                                spannedProcessor.getTimeSpan().getEndTime()));
                            }
                            StoragePlugin storagePlugin = dataSource.getStoragePlugin();
                            unitsofretrieval.add(new UnitOfRetrieval(
                                    storagePlugin.getDescription(),
            ...
            if (RetrievalState.includeExternalServers(req)) {
                ...
                if (failoverServer != null) {
                    logger.debug("Including the failover server " + failoverServer + " during data retrieval for PV "
                            + pvName);
                    String pluginDefString = "pbraw://localhost?name=failover&rawURL="
                            + URLEncoder.encode(
                    ...
                    if (failoverStrms != null && !failoverStrms.isEmpty()) {
                        ...
                        for (Callable<EventStream> failoverStrm : failoverStrms) {
                            try (EventStream ev = failoverStrm.call()) {
                                ...
                                if (cacheStream == null) {
                                    cacheStream = new ArrayListEventStream(
                                            10000, (RemotableEventStreamDesc) ev.getDescription());
                                }
                                for (Event e : ev) {
                                    cacheStream.add(e);
                                    sampleCount++;
                        ...
                        if (sampleCount > 0) {
                            ...
                            for (UnitOfRetrieval unitofretrieval : unitsofretrieval) {
                                ...
                                unitofretrieval.wrapWithFailoverStreams(
                                        CallableEventStream.makeOneStreamCallableList(cacheStream));
                            }
                        } else {
