...
public class ArchiverValuesHandler implements XMLRPCStaxProcessor, EventStream, Iterator<Event>, RemotableOverRaw {
    ...
    private final ArchDBRTypes expectedDBRType;
     ...
     */
    public ArchiverValuesHandler(String pvName, InputStream is, String source, ArchDBRTypes expectedDBRType)
            throws IOException {
        this.pvName = pvName;
        this.source = source;
        try {
    ...
    @Override
    public boolean startElement(String localName) {
        currentNodes.add(localName);

        if (inValues) {
            // The Value portion of the XML document is an Array of Structs.
            // If we are in the Values processing portion, we create a new working copy every time we encounter a
            // struct.
    ...
    @Override
    public boolean endElement(String localName, String value) {
        boolean continueProcessing = true;
        ...
        boolean lastTwoEqualsCheck = lastTwoNodes.equals("value.i4")
                || lastTwoNodes.equals("value.string")
                || lastTwoNodes.equals("value.double");
        if (inMeta) {
            if (localName.equals("struct")) {
        ...
        if (!inMeta && lastName != null) {
            ...
            if (serializingConstructor == null) {
                // The caller has passed in an expected DBR Type perhaps because we are archiving in the appliance as
                // this type.
                if (this.expectedDBRType != null) {
                    ...
                    if (valueType != null && elementCount != -1) {
                        logger.debug("Inferring dbrtype from the value type and element count" + getValueType()
                                + " and " + getElementCount());
                        dbrType = getValueType().getDBRType(getElementCount());
                    }
                }
                if (dbrType != null) {
                    serializingConstructor =
        ...
        if (inValues) {
            if (localName.equals("struct") && (workingCopyOfEvent != null)) {
                ...
                try {
                    currentEvent = (DBRTimeEvent)
                            serializingConstructor.newInstance(new HashMapEvent(dbrType, workingCopyOfEvent));
                    ...
                    if (previousEventEpochSeconds > 0) {
                        if (currentEventEpochSeconds < previousEventEpochSeconds) {
                            logger.error("Skipping decreasing timestamp from CA "
                                    + TimeUtils.convertToHumanReadableString(currentEventEpochSeconds)
                                    + " and previous "
    ...
    public enum ArchiverValuesType {
        ...
        static final HashMap<Integer, ArchiverValuesType> int2Enum = new HashMap<>();
