...
public class ArchiverValuesHandler implements XMLRPCStaxProcessor, EventStream, Iterator<Event>, RemotableOverRaw {
    ...
    public enum ArchiverValuesType {
        ...
        ArchDBRTypes getDBRType(int elementCount) {
            if (elementCount > 1) {
                switch (this) {
                    case CHANNEL_ARCHIVER_STRING:
                        return ArchDBRTypes.DBR_WAVEFORM_STRING;
                    case CHANNEL_ARCHIVER_DOUBLE:
                        return ArchDBRTypes.DBR_WAVEFORM_DOUBLE;
                    case CHANNEL_ARCHIVER_ENUM:
                        return ArchDBRTypes.DBR_WAVEFORM_ENUM;
                    case CHANNEL_ARCHIVER_INT:
                        return ArchDBRTypes.DBR_WAVEFORM_INT;
                    default:
                        throw new RuntimeException(
                                "Cannot map " + this + " element count " + elementCount + " to an ArchDBRTypes");
                }
            } else {
                switch (this) {
                    case CHANNEL_ARCHIVER_STRING:
