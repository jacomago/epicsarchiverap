...
public class GetClientConfiguration implements BPLAction {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse resp, ConfigService configService)
        ...
        if (configFileName == null || configFileName.isEmpty() || configFileName.contains("..")) {
        ...
        if (!Files.exists(configFilePath)) {
            logger.error(
                    "The archive viewer config file does not exist on the file system " + configFilePath.toString());
            resp.sendError(HttpServletResponse.SC_NOT_FOUND);
            return;
        }
