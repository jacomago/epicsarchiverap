                ...
                if (externalServers != null) {
                    for (String serverUrl : externalServers.keySet()) {
                        ...
                        if (index.equals("pbraw")) {
                            logger.debug("Asking external EPICS Archiver Appliance " + serverUrl + " for PV's matching "
                                    + nameToMatch);
                            pvNamesURLs.add(serverUrl + "/bpl/getMatchingPVs?skipExternalServers=true&regex="
                                    + URLEncoder.encode(nameToMatch, "UTF-8") + "&limit=" + Integer.toString(limit));
                        }
                    }
                }
            } else {
