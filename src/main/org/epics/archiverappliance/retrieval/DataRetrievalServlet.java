...
public class DataRetrievalServlet extends HttpServlet {
    ...
    private RequestTimes getRequestTimes(HttpServletRequest req, HttpServletResponse resp, String pvName)
            throws IOException {
        // ISO datetimes are of the form "2011-02-02T08:00:00.000Z"
        Instant end;
        try {
        ...
        if (end.isBefore(start)) {
            logAndRespond(
                    "For request, end " + end + " is before start " + start + " for pv " + pvName,
                    null,
                    resp,
    ...
    private RequestTimes getRequestTimes(
            HttpServletRequest req, HttpServletResponse resp, String pvName, Instant end, Instant start)
            throws IOException {
                ...
                try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
                    ...
                    try (CloseableHttpResponse response = httpclient.execute(getMethod)) {
                        if (response.getStatusLine().getStatusCode() == 200) {
                            ...
                            GetUrlContent.handleHeaders(resp, response, entity, proxiedHeaders, logger);
                        } else {
     ...
     * @throws IOException
     */
    private boolean parseTimeRanges(
            HttpServletResponse resp, String pvName, LinkedList<TimeSpan> requestTimes, String timeRangesStr)
            throws IOException {
