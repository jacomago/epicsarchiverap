...
class MergeDedupConsumer implements EventStreamConsumer, AutoCloseable {
    ...
    public void consumeEventStream(EventStream strm) throws Exception {
        // This while true looks scary but 99.99% of the time we should hit the return and get out with one execution of
        // the loop
        ...
        while (true) {
            try {
                ...
                if (logger.isDebugEnabled())
                    logger.debug(
                            "Got a change in years exception. Inserting a fake swicthingToStream with the same stream. Previous="
                                    + ex.getPreviousYear() + " and current=" + ex.getCurrentYear());
            }
        }
    }
        ...
        try {
            ...
            for (Event e : strm) {
                try {
                    ...
                    if (!haveIpushedTheFirstEvent && firstEvent == null) {
                        logger.debug("Making a copy of the first event "
                    ...
                    if (!haveIpushedTheFirstEvent) {
                        if (e.getEventTimeStamp().isBefore(this.startTimeStamp)
                                || e.getEventTimeStamp().equals(this.startTimeStamp)) {
                            logger.debug("Making a copy of another event "
                                    + TimeUtils.convertToHumanReadableString(e.getEventTimeStamp()));
                            firstEvent = e.makeClone();
                        } else {
                            haveIpushedTheFirstEvent = true;
                            logger.debug("Consuming first and current events "
                                    + TimeUtils.convertToHumanReadableString(e.getEventTimeStamp()));
                            mimeresponse.consumeEvent(firstEvent);
                            ...
                            if (!e.getEventTimeStamp().isAfter(timestampOfLastEvent)) {
                                logger.debug(
                                        "After sending first event, current event is not after the first event. Skipping "
                                                + TimeUtils.convertToHumanReadableString(e.getEventTimeStamp()));
                                skippedEvents++;
                            } else {
            ...
            if (eventsInCurrentStream == 0) {
                logger.info("The stream from "
                        + ((strm.getDescription() != null)
                                ? strm.getDescription().getSource()
                                : "Unknown") + " was an empty stream.");
            }
