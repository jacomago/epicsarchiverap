        ...
        if (!countAddedToStream) {
            if (data == null) {
                RemotableEventStreamDesc desc = new RemotableEventStreamDesc(
                        ArchDBRTypes.DBR_SCALAR_INT, pvName, TimeUtils.computeYearForEpochSeconds(startTime / 1000));
                data = new ArrayListEventStream(1, desc);
            }
            data.add(new POJOEvent(
                    ArchDBRTypes.DBR_SCALAR_INT, Instant.ofEpochMilli(startTime), new ScalarValue<>(count), 0, 0));
