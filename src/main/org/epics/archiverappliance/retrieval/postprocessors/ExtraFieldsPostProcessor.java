...
public class ExtraFieldsPostProcessor implements PostProcessor {
    ...
    public long estimateMemoryConsumption(
        ...
        int estimatedDays =
                1 + ((int) (TimeUtils.convertToEpochSeconds(end) - TimeUtils.convertToEpochSeconds(start)) / 86400);
                ...
                try (EventStream strm = callable.call()) {
                    ...
                    for (Event e : strm) {
                        ...
                        if (tev.hasFieldValues()) {
                            Map<String, String> extraFields = tev.getFields();
                            if (extraFields.containsKey(fieldName)) {
                                POJOEvent newEvent = new POJOEvent(
                                        tev.getDBRType(),
                                        tev.getEventTimeStamp(),
                                        tev.getFieldValue(fieldName),
                                        tev.getStatus(),
                                        tev.getSeverity());
                                buf.add(newEvent);
                            }
