...
public class SummaryStatsCollectorEventStream implements EventStream, RemotableOverRaw {
    ...
    public SummaryStatsCollectorEventStream(
            long firstBin,
            long lastBin,
            int intervalSecs,
            RemotableEventStreamDesc desc,
            LinkedHashMap<Long, SummaryValue> consolidatedData,
            boolean inheritValuesFromPreviousBins,
            boolean zeroOutEmptyBins,
            boolean vectorType,
            int elementCount) {
                ...
                if (foundValue) {
                    ...
                    if (summaryValue.additionalCols != null && !summaryValue.additionalCols.isEmpty()) {
                        for (String addnName : summaryValue.additionalCols.keySet()) {
                            ...
                            fieldValues.put(addnName, addnValue);
                        }
                    }

                    strm.add(pojoEvent.cloneWithExtraFieldValues(fieldValues, false));
                    if (currentYear == -1) {
