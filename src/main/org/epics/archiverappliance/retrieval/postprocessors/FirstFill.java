...
public class FirstFill implements PostProcessor, PostProcessorWithConsolidatedEventStream {
    private static Logger logger = LogManager.getLogger(FirstFill.class.getName());
    ...
    public void initialize(String userarg, String pvName) throws IOException {
        if (userarg != null && userarg.contains("_")) {
            ...
            logger.debug("FirstFill, using the default interval of  " + intervalSecs
                            ...
                            if (!lastSampleBeforeStartAdded && lastSampleBeforeStart != null) {
                                logger.debug("Adding lastSampleBeforeStart to bin "
                                        + TimeUtils.convertToHumanReadableString(
                                                lastSampleBeforeStart.getEpochSeconds()));
                            ...
                            if (binNumber != currentBin) {
                                ...
                                if (!bin2Event.containsKey(currentBin)) {
                                    ...
                                    if (currentBin == firstBin) {
                                        // If this is the first bin, we replace the existing value only if event is from
                                        // a previous bin before start time
                                        // See note below
    ...
    public EventStream getConsolidatedEventStream() {
        ...
        if (bin2Event.isEmpty()) {
            ...
            return new FillsCollectorEventStream(
                    this.firstBin == 0 ? 0 : this.firstBin - 1, lastBin, intervalSecs, srcDesc, bin2Event);
        }
    }
