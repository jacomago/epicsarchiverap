 ...
 *
 */
public class Fliers implements PostProcessor {
    private static final float DEFAULT_DEVIATIONS = 3.0f;
    private static Logger logger = LogManager.getLogger(Fliers.class.getName());
    ...
    public void initialize(String userarg, String pvName) throws IOException {
        try {
            if (userarg != null && userarg.contains("_")) {
                ...
                logger.debug("Using the default interval of  " + intervalSecs + " and " + numDeviations
                        + " deviations as the user has not specified the interval argument.");
            }
        } catch (Exception ex) {
    ...
    public Callable<EventStream> wrap(final Callable<EventStream> callable) {
        return new Callable<EventStream>() {
            ...
            private void addStat(
                    SummaryStatistics stats, ArrayListEventStream buf, long binNumber, ArrayListEventStream ret) {
                ...
                for (Event e : buf) {
                    double val = e.getSampleValue().getValue().doubleValue();
                    // This should be the only difference between ignore fliers and fliers
                    if (Math.abs(val - mean) > cutoff) {
                        ret.add(e);
                    } else {
                        // if(logger.isDebugEnabled()) logger.debug("Skipping value " + val + " based on mean=" + mean +
                        // " and stdz = " + stdz + " for timestamp " +
