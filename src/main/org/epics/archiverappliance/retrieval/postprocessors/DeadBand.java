        ...
        return new Callable<EventStream>() {
            ...
            public EventStream call() throws Exception {
                try (EventStream strm = callable.call()) {
                    ArrayListEventStream buf =
                            new ArrayListEventStream(0, (RemotableEventStreamDesc) strm.getDescription());
                    for (Event e : strm) {
                        try {
                            // The very first sample we get we write out and that becomes the "last" value.
                            if (lastEventWrittenOut == null) {
                                ...
                                buf.add(clone);
                                logger.debug("Writing the very first sample out at "
                                        + TimeUtils.convertToISO8601String(clone.getEventTimeStamp()));
                                continue;
                            }

                            double currentVALue = e.getSampleValue().getValue().doubleValue();
                            double lastVALueWrittenOut = lastEventWrittenOut
                                    .getSampleValue()
                                    .getValue()
                                    .doubleValue();
                            ...
                            if (Double.isInfinite(currentVALue)
                                    ...
                                    || Double.isInfinite(lastVALueWrittenOut)
                                    || Double.isNaN(lastVALueWrittenOut)) {
                                // If either current value or previous value is NAN/INF, unconditionally write out
                                // current value and update last known value
                                Event clone = e.makeClone();
                                lastEventWrittenOut = clone;
                                buf.add(clone);
                                logger.debug("Writing out from infinity/NAN at "
                                        + TimeUtils.convertToISO8601String(clone.getEventTimeStamp()));
                                continue;
                            }
                            ...
                            if (currentSeverity != previousSeverity) {
                                ...
                                lastEventWrittenOut = clone;
                                buf.add(clone);
                                logger.debug("Writing out from severity change at "
                                        + TimeUtils.convertToISO8601String(clone.getEventTimeStamp()));
                                continue;
                            }

                            boolean currentConnectionChangeStatus = currDBRTimeEvent.hasFieldValues()
                                    && currDBRTimeEvent.getFields().containsKey(CONNECTION_LOST_SECONDS_KEY);
                            boolean previousConnectionChangeStatus = previousDBRTimeEvent.hasFieldValues()
                                    && previousDBRTimeEvent.getFields().containsKey(CONNECTION_LOST_SECONDS_KEY);
                            if (currentConnectionChangeStatus != previousConnectionChangeStatus) {
                                ...
                                lastEventWrittenOut = clone;
                                buf.add(clone);
                                logger.debug("Writing out from connection state change at "
                                        + TimeUtils.convertToISO8601String(clone.getEventTimeStamp()));
                                continue;
                            }
                            ...
                            if (diff > adelValue) {
                                ...
                                lastEventWrittenOut = clone;
                                buf.add(clone);
                                logger.debug("Writing out from magnitude change at "
                                        + TimeUtils.convertToISO8601String(clone.getEventTimeStamp()));
                                continue;
                            }
