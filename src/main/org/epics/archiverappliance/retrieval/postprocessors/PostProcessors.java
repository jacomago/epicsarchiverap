...
public class PostProcessors {
    ...
    private enum InheritValuesFromPreviousBins {
        DO_NOT_INHERIT,
        INHERIT
    }
    ...
    private static class PostProcessorImplementation {
        ...
        public PostProcessorImplementation(
                String key,
                Class<? extends PostProcessor> clazz,
                InheritValuesFromPreviousBins inheritValuesFromPreviousBins) {
            this.key = key;
            this.clazz = clazz;
    ...
    private static LinkedList<PostProcessorImplementation> postprocessors =
            new LinkedList<PostProcessorImplementation>();
    ...
    private static void registerPostProcessor(String identity, Class<? extends PostProcessor> clazz) {
        try {
            ...
            if (implementationInstance instanceof FillNoFillSupport) {
                ...
                postprocessors.add(new PostProcessorImplementation(
                        identity + "Sample", clazz, InheritValuesFromPreviousBins.DO_NOT_INHERIT));
            }
        } catch (Exception e) {
            ...
            try {
                for (PostProcessorImplementation implementation : postprocessors) {
                    if (postProcessorUserArg.startsWith(implementation.key)) {
                        ...
                        PostProcessor implementationInstance =
                                implementation.clazz.getConstructor().newInstance();
