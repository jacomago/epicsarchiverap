...
public class TwoWeekRaw implements PostProcessor, TimeSpanDependentProcessing {
    ...
    public List<TimeSpanDependentProcessor> generateTimeSpanDependentProcessors(List<TimeSpan> timeSpans) {
        long twoWeeksAgo = TimeUtils.convertToEpochSeconds(TimeUtils.now())
                - 2 * 7 * PartitionGranularity.PARTITION_DAY.getApproxSecondsPerChunk();
        ...
        for (TimeSpan timeSpan : timeSpans) {
            ...
            if (endSeconds <= twoWeeksAgo) {
                ...
                logger.debug("Returning sparsified+raw data for pv " + pvName + " breaking at "
                        + TimeUtils.convertToHumanReadableString(twoWeeksAgoTs));
                ret.add(new TimeSpanDependentProcessor(
                        new TimeSpan(timeSpan.getStartTime(), twoWeeksAgoTs), sparsifiedData));
