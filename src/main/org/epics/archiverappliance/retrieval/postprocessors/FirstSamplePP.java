...
public class FirstSamplePP implements PostProcessor, AfterAllStreams {
    ...
    public void initialize(String userarg, String pvName) throws IOException {
        if (userarg != null && userarg.contains("_")) {
            ...
            logger.debug("FirstSamplePP, using the default interval of  " + intervalSecs
                            ...
                            if (binNumber >= firstBin && binNumber <= lastBin) {
                                if (binNumber != previousBinNum) {
                                    if (!lastSampleBeforeStartAdded && lastSampleBeforeStart != null) {
                                        logger.info("Adding the lastSampleBeforeStart at "
                                                + TimeUtils.convertToHumanReadableString(
                                                        lastSampleBeforeStart.getEventTimeStamp())
                                    ...
                                    logger.debug(
                                            "Bin Number " + binNumber + " First: " + firstBin + " Last: " + lastBin);
                                }
                                ...
                                if (!lastSampleBeforeStartAdded) {
                                    if (lastSampleBeforeStart != null) {
                                        if (e.getEpochSeconds() >= lastSampleBeforeStart.getEpochSeconds()) {
                                            ...
                                            logger.info("Resetting the lastSampleBeforeStart to "
                                                    + TimeUtils.convertToHumanReadableString(
                                                            lastSampleBeforeStart.getEventTimeStamp()));
                                        }
                                    } else {
                                        lastSampleBeforeStart = e.makeClone();
                                        lastSampleDesc = strm.getDescription();
                                        logger.info("Setting the lastSampleBeforeStart to "
                                                + TimeUtils.convertToHumanReadableString(
                                                        lastSampleBeforeStart.getEventTimeStamp()));
                                    }
                                }
    ...
    public EventStream anyFinalData() {
        if (!lastSampleBeforeStartAdded && lastSampleBeforeStart != null) {
            ...
            logger.info("Returning the lastSampleBeforeStart at "
                    + TimeUtils.convertToHumanReadableString(lastSampleBeforeStart.getEventTimeStamp())
                    + " into the result stream after all the other streams have been processed.");
            buf.add(lastSampleBeforeStart);
            lastSampleBeforeStartAdded = true;
            lastSampleDesc = null;
            return buf;
