...
import static org.epics.archiverappliance.engine.pv.PVMetrics.CONNECTION_REGAINED_SECONDS_KEY;
        ...
        return new Callable<EventStream>() {
            ...
            public EventStream call() throws Exception {
                try (EventStream strm = callable.call()) {
                    ...
                    if (srcDesc == null)
                        srcDesc = new RemotableEventStreamDesc(
                                ArchDBRTypes.DBR_SCALAR_DOUBLE, strmDesc.getPvName(), strmDesc.getYear());
                    for (Event e : strm) {
                        try {
                            ...
                            if (binNumber >= firstBin && binNumber <= lastBin) {
                                ...
                                // different rates for PVs, the bin number for the starting bin could be different...
                                // We could add a firstbin-1 and put all values before the starting timestamp in that
                                // bin but that would give incorrect summaries.
                                ...
                                if (dbrTimeEvent.hasFieldValues()
                                        && dbrTimeEvent.getFields().containsKey(CONNECTION_REGAINED_SECONDS_KEY)) {
    ...
    class PlotBin {
        ...
        public void outputEvents(ArrayListEventStream ret) {
            ...
            if (lastSample == null) {
                ret.add(makeEvent(
                        SampleLocation.ASIS,
                        firstSample.getEpochSeconds(),
                        firstSample.getSampleValue().getValue().doubleValue()));
                return;
            }
            ...
            if (stats.getN() == 2) {
                ...
                ret.add(makeEvent(
                        SampleLocation.ASIS,
                        lastSample.getEpochSeconds(),
                        lastSample.getSampleValue().getValue().doubleValue()));
                return;
            }
            ...
            ret.add(makeEvent(
                    SampleLocation.FIRSTSAMPLE,
                    binStartEpochSeconds,
                    firstSample.getSampleValue().getValue().doubleValue()));
        ...
        private Event makeEvent(SampleLocation location, long binStartEpochSeconds, double value) {
            ...
            // logger.info("Location " + location.toString() + " epochSeconds " + epochSeconds + " for bin " + binNumber
            // + " and intervalSecs " + intervalSecs);
            POJOEvent pojoEvent = new POJOEvent(
                    ArchDBRTypes.DBR_SCALAR_DOUBLE,
                    TimeUtils.convertFromEpochSeconds(epochSeconds, 0),
                    new ScalarValue<Double>(value),
                    0,
                    this.maximizedSeverity);
