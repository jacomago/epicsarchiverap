    ...
    public List<DataSourceforPV> getDataSources(
        ...
        if (typeInfo == null) {
            ...
            if (caServers != null) {
                ...
                for (ChannelArchiverDataServerPVInfo caServer : caServers) {
                    ...
                    dataSourcesForPV.add(new DataSourceforPV(
                            pvName, caServer.getServerInfo().getPlugin(count, howStr), lifetimeid++, null, null));
        ...
        try {
            ...
            if ((endEpochSeconds >= currentEpochSeconds)
                    || ((currentEpochSeconds - endEpochSeconds) < 2 * engineWriteThreadInSeconds)) {
                ...
                logger.debug(
                        "Skipping going to the engine for data " + TimeUtils.convertToISO8601String(currentEpochSeconds)
                                + "/" + TimeUtils.convertToISO8601String(endEpochSeconds));
            }
            ...
            // Add any external servers if any only if the creation time for this type info is after the start time of
            // the request.
                ...
                if (creationTime == null || start.isBefore(creationTime)) {
                    ...
                    if (caServers != null) {
                        for (ChannelArchiverDataServerPVInfo caServer : caServers) {
                            ...
                            dataSourcesForPV.add(new DataSourceforPV(
                                    pvName,
                                    caServer.getServerInfo().getPlugin(count, howStr),
                                    lifetimeid++,
                                    start,
                                    creationTime));
                        }
