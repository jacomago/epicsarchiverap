...
public class PvaGetPVData implements PvaAction {
    ...
    private PVAStructure doGetSinglePV(Map<String, String> reqParameters, ConfigService configService)
        ...
        if (end.isBefore(start)) {
            String msg =
                    "For request, end " + end.toString() + " is before start " + start.toString() + " for pv " + pvName;
            logger.error(msg);
            throw new PvaActionException(msg);
        }
        ...
        try {
            ...
            throw new PvaActionException("Postprocessor threw an exception during initialization for " + pvName, ex);
        }

        // TODO creating the result structure that will be sent over the wire.
        PVAStructure result =
                new PVAStructure("result", "NTComplexTable", new PVAStringArray("labels"), new PVAAnyArray("value"));
                ...
                try (EventStream eventStream = future.get()) {
                    ...
                    try {
                        ...
                        if (ex != null && ex.toString() != null && ex.toString().contains("ClientAbortException")) {
                            ...
                            // in the build path.
                            logger.info(
                                    "Exception when consuming and flushing data from " + sourceDesc.getSource(), ex);
                        } else {
                            logger.error(
    ...
    private PVAStructure doGetMultiPV(Map<String, String> reqParameters, ConfigService configService)
        ...
        String timeRangesStr = reqParameters.get("timeranges");
        if (timeRangesStr != null) {
            parseTimeRanges("[" + StringUtils.join(pvNames, ", ") + "]", requestTimes, timeRangesStr);
        ...
        for (int i = 0; i < pvNames.size(); i++) {
            ...
            if (postProcessors.get(i) == null) {
                if (useReduced) {
                    ...
                    logger.debug("Using the default usereduced preprocessor " + defaultPPClassName);
                    try {
                        postProcessors.set(i, (PostProcessor) Class.forName(defaultPPClassName)
                                .getConstructor()
                                .newInstance());
                    } catch (Exception ex) {
                        logger.error("Exception constructing new instance of post processor " + defaultPPClassName, ex);
     ...
     * @param timeRangesStr
     */
    private void parseTimeRanges(String pvName, LinkedList<TimeSpan> requestTimes, String timeRangesStr)
            throws PvaActionException {
        ...
        while (!timeRangesList.isEmpty()) {
            ...
            if (t1.isBefore(t0)) {
                String msg =
                        "For request, end " + t1.toString() + " is before start " + t0.toString() + " for pv " + pvName;
                logger.error(msg);
                throw new PvaActionException(msg);
            }
    ...
    private PvaMergeDedupConsumer createMergeDedupConsumer(PVAStructure result) throws ServletException {
        ...
        try {
            PvaMimeResponse mimeresponse =
                    PvaMimeResponse.class.getConstructor().newInstance();
    ...
    private HashMap<String, String> fetchLatestMedataFromEngine(String pvName, ApplianceInfo applianceForPV) {
        try {
            String metadataURL =
                    applianceForPV.getEngineURL() + "/getMetadata?pv=" + URLEncoder.encode(pvName, "UTF-8");
    ...
    private static RetrievalExecutorResult determineExecutorForPostProcessing(
            String pvName, PVTypeInfo typeInfo, LinkedList<TimeSpan> requestTimes, PostProcessor postProcessor) {
    ...
    private LinkedList<Future<RetrievalResult>> resolveAllDataSources(
        ...
        for (TimeSpan timespan : executorResult.requestTimespans) {
            ...
            LinkedList<UnitOfRetrieval> unitsofretrieval = datasourceresolver.resolveDataSources(
                    pvName,
