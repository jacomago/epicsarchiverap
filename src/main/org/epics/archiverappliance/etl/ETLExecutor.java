...
public class ETLExecutor {
    private static final Logger logger = LogManager.getLogger(ETLExecutor.class.getName());

    /**
     * This should only be called from within unit tests...
     *
     * @param configService ConfigService
     * @param etlRunTime    Instant
     * @throws IOException &emsp;
     */
    public static void runETLs(ConfigService configService, Instant etlRunTime) throws IOException {
        for (String pvName : configService.getPVsForThisAppliance()) {
            ...
            for (ETLPVLookupItems lookupItem : lookupItems) {
                ...
                ETLJob job = new ETLJob(lookupItem, etlRunTime);
                job.run();
