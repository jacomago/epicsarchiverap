...
public class StorageWithLifetime {
    ...
    int minPartitionSourceGranularityInSecs = 366
            * PartitionGranularity.PARTITION_DAY
                    .getApproxSecondsPerChunk(); // Init to a large value; below we use Math.min to pick the correct
    // value.
        ...
        for (StorageWithLifetime storage : finalStorages) {
            try {
                ETLMetricsForLifetime metricsForLifetime =
                        configService.getETLLookup().getApplianceMetrics().get(storage.lifetimeid);
    ...
    public static String getStorageDetails(ConfigService configService) {
        ...
        for (StorageWithLifetime storage : finalStorages) {
            ...
            try {
                ...
                detail.put(
                        "available_space_percent", twoSignificantDigits.format(availbleSpaceGB * 100 / totalSpaceGB));
                double avgTimeIntoThisDestInMillis =
                        storage.totalETLTimeIntoThisDestInMillis / storage.maxTotalETLRunsIntoThisDest;
                detail.put(
                ...
                if (etlSrc instanceof StorageMetrics) {
                    ...
                    if (!storages.containsKey(storageMetricsAPI.getName())) {
                        storages.put(
                                ...
                                new StorageWithLifetime(storageMetricsAPI, lookupItem.getLifetimeorder()));
                    }
                }
