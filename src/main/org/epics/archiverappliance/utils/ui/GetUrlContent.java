...
/**
 * Small utility for getting the contents of a URL as various things
 * @author mshankar
 *
 */
public class GetUrlContent {
    ...
    public static JSONObject postDataAndGetContentAsJSONObject(String url, List<JSONObject> array) throws IOException {
        try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
            HttpEntity entity = postArray(url, array, httpclient);
            if (entity != null) {
    ...
    private static HttpEntity postArray(String url, List<JSONObject> array, CloseableHttpClient httpclient)
            throws IOException {
        HttpResponse response = postMethod(url, httpclient, JSONValue.toJSONString(array));

        return response.getEntity();
    }

    private static HttpResponse postMethod(String url, CloseableHttpClient httpclient, String jsonString)
            throws IOException {
        ...
        StringEntity archiverValues = new StringEntity(jsonString, ContentType.APPLICATION_JSON);
    ...
    public static JSONArray postDataAndGetContentAsJSONArray(String url, LinkedList<JSONObject> array)
            throws IOException {
        return (JSONArray) postAndGetContent(url, array);
    }
     ...
     * @throws IOException  &emsp;
     */
    public static JSONArray postDataAndGetContentAsJSONArray(String url, JSONArray array) throws IOException {
        return (JSONArray) postAndGetContent(url, array);
    }
    ...
    public static JSONObject postObjectAndGetContentAsJSONObject(String url, JSONObject object) throws IOException {
        return (JSONObject) postAndGetContent(url, object);
    }
     ...
     * @throws IOException  &emsp;
     */
    public static Object postAndGetContent(String url, Object object) throws IOException {
        try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
            HttpResponse response = postMethod(url, httpclient, JSONValue.toJSONString(object));
            HttpEntity entity = response.getEntity();
            if (entity != null) {
                logger.debug("Obtained a HTTP entity of length " + entity.getContentLength());
                // ArchiverValuesHandler takes over the burden of closing the input stream.
                try (InputStream is = entity.getContent()) {
                    return JSONValue.parse(new InputStreamReader(is));
                }
            } else {
                throw new IOException("HTTP response did not have an entity associated with it");
            }
        ...
        try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
            ...
            try (CloseableHttpResponse response = httpclient.execute(getMethod)) {
                if (response.getStatusLine().getStatusCode() == 200) {
                    ...
                    handleHeaders(resp, response, entity, proxiedHeaders, logger);
                } else {
                    logger.error(
