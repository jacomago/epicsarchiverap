...
public class StreamPBIntoOutput {
     ...
     */
    public static int streamPBIntoOutputStream(EventStream st, OutputStream os, Instant start, Instant end)
            throws IOException {
        long startTimeInEpochSeconds = 0;
        if (start != null) startTimeInEpochSeconds = TimeUtils.convertToEpochSeconds(start);
        long endTimeInEpochSeconds = Long.MAX_VALUE;
        if (end != null) endTimeInEpochSeconds = TimeUtils.convertToEpochSeconds(end);
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("Skipping event"
                                ...
                                + TimeUtils.convertToISO8601String(
                                        TimeUtils.convertFromEpochSeconds(startTimeInEpochSeconds, 0))
                                + " with end "
                                + TimeUtils.convertToISO8601String(
                                        TimeUtils.convertFromEpochSeconds(endTimeInEpochSeconds, 0)));
                    }
                }
            }
    ...
    private static short writeHeader(
            short previousYear, OutputStream os, RemotableEventStreamDesc desc, Event e, long startTimeInEpochSeconds)
            throws IOException {
        short currentYear = -1;
        if (e != null) {
        ...
        if (previousYear == -1 || previousYear != currentYear) {
            if (previousYear != -1)
                os.write(
                        LineEscaper
                                .NEWLINE_CHAR); // Blank line indicates a new chunk in the event stream; skip if this is
            // the first chunk
