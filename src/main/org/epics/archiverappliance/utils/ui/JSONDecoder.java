...
public class JSONDecoder<T> {
    ...
    private JSONDecoder(Class<T> clazz) throws IntrospectionException, NoSuchMethodException {
        ...
        for (PropertyDescriptor descriptor : descriptors) {
            ...
            if (descriptor.getPropertyType().equals(String.class)) {
                ...
                throw new IntrospectionException("Do not have JSON decoder for property " + descriptor.getName()
                        + " of type " + descriptor.getPropertyType().getCanonicalName());
            }
        }
    }

    public void decode(JSONObject jsonObj, T obj)
    ...
    private static interface AttributeDecoder<T> {
        void decode(JSONObject jsonObj, T obj)
                throws IllegalAccessException, InvocationTargetException, InstantiationException;
    }

    private static String WRITE_METHOD_EXPLANATION =
            ". Our custom JSON decoder requires both get and set methods in standard bean syntax. Use Eclipse to add a 'standard' set method";
    ...
    private static class StringConstructor<T> implements AttributeDecoder<T> {
        ...
        private StringConstructor(PropertyDescriptor descriptor) throws NoSuchMethodException {
            ...
            if (writeMethod == null)
                throw new NoSuchMethodException("No write method for " + propertyName + WRITE_METHOD_EXPLANATION);
    ...
    private static class ArrayOfStringsDecoder<T> implements AttributeDecoder<T> {
        ...
        private ArrayOfStringsDecoder(PropertyDescriptor descriptor) throws NoSuchMethodException {
            ...
            constructorFromString =
                    descriptor.getPropertyType().getComponentType().getConstructor(String.class);
        }
    ...
    private static class EnumDecoder<T> implements AttributeDecoder<T> {
        ...
        private EnumDecoder(PropertyDescriptor descriptor, Class<? extends Enum> enumClass)
