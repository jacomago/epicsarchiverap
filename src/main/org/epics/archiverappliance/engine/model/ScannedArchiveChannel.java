...
public class ScannedArchiveChannel extends ArchiveChannel implements Runnable {
     ...
     */
    public ScannedArchiveChannel(
            final String name,
            final Writer writer,
            Enablement enablement,
            final int buffer_capacity,
            final Instant last_timeestamp,
        ...
        double scanJitterFactor = Double.parseDouble((String) configservice
                .getInstallationProperties()
                .getOrDefault("org.epics.archiverappliance.engine.epics.scanJitterFactor", "0.95"));
            ...
            if (isEnabled()) {
                try {
                    ...
                    if (isLessThanScanPeriod(lastDBRTimeEvent, latestDBRTimeEvent)) {
                        ...
                        // We however keep track of the server time when we got a handle event for comparision in the
                        // SCAN thread.
    ...
    private boolean isMatchingTimeStamp(final DBRTimeEvent tempEvent1, final DBRTimeEvent tempEvent2) {
        if (tempEvent1 != null
                && tempEvent2 != null
                && tempEvent1.getEventTimeStamp() != null
                && tempEvent2.getEventTimeStamp() != null) {
    ...
    private boolean isLessThanScanPeriod(final DBRTimeEvent tempEvent1, final DBRTimeEvent tempEvent2) {
        if (tempEvent1 != null
            ...
            Instant time1 = tempEvent1.getEventTimeStamp();
            Instant time2 = tempEvent2.getEventTimeStamp();
            // logger.debug("Diff = " + (time2.toEpochMilli() - time1.toEpochMilli()) + " and scanPeriodMillis " +
            // scanPeriodMillis);
