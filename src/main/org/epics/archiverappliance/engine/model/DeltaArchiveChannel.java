...
public class DeltaArchiveChannel extends ArchiveChannel {
    ...
    public String getMechanism() {
        return "on delta [" + PeriodFormat.formatSeconds(period_estimate) + ", " + delta + "]";
    }
    ...
    private boolean isBeyondDelta(final DBRTimeEvent timeevent) {
        ...
        // Archive NaN, Inf'ty
        if (Double.isNaN(number)) return true;
        double previous;
        synchronized (this) {
            // Anything to compare against?
            if (lastDBRTimeEvent == null) return true;
