...
public class DroppedEventsTimestampReport implements BPLAction {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse resp, ConfigService configService)
        ...
        List<PVDroppedEvents> eventRates = getDroppedEventsTimeStamp(configService, limit);
    ...
    private static List<PVDroppedEvents> getDroppedEventsTimeStamp(ConfigService configService, String limit) {
        ...
        for (ArchiveChannel channel : engineContext.getChannelList().values()) {
            ...
            if (pvMetrics.getTimestampWrongEventCount() > 0) {
                eventRates.add(new PVDroppedEvents(channel.getName(), pvMetrics.getTimestampWrongEventCount()));
            }
        }
