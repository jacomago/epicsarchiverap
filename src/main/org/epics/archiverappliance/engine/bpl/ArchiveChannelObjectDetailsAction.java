 ...
 *
 */
public class ArchiveChannelObjectDetailsAction implements BPLAction<ConfigService> {
    ...
    public HashMap<String, Object> objectToJSON(String pvName, Object obj, boolean useSuperClassFields)
        ...
        logger.debug("Channel for " + pvName + " of class " + theClass.getCanonicalName() + " has " + objFields.length
                + " declared fields");
        for (Field objField : objFields) {
            ...
            if (objField.getType().isPrimitive() || objField.getType().isEnum()) {
                ...
                if (timeEvent != null) {
                    objValues.put(
                            objField.getName(), TimeUtils.convertToHumanReadableString(timeEvent.getEventTimeStamp()));
                }
