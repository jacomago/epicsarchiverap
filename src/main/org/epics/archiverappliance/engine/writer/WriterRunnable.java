...
public class WriterRunnable implements Runnable {
    ...
    public WriterRunnable(ConfigService configservice) {
        ...
        int limit = configservice.getEngineContext().getWriteThreadCount();
     ...
     * Called directly from the year listener (CA ingestion thread). Submits the blocking I/O
     * to the virtual thread executor. Skips submission if a year-change write for this channel
     * is already in-flight, preventing duplicate writes across a year boundary.
     */
    private void writeYearChange(SampleBuffer buffer) {
        if (writeExecutor.isShutdown()) {
            logger.warn("Skipping year-change flush for {} — executor already shut down",
                    buffer.getChannelName());
            return;
        }
