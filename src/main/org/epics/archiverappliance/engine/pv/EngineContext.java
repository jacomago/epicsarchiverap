...
public class EngineContext {
    ...
    public EngineContext(final ConfigService configService) {
        ...
        MAXIMUM_DISCONNECTED_CHANNEL_PERCENTAGE_BEFORE_STARTING_METACHANNELS = Double.valueOf(configService
        ...
        METACHANNELS_TO_START_AT_A_TIME = Integer.valueOf(configService
                .getInstallationProperties()
                .getProperty(
                        "org.epics.archiverappliance.engine.metachannels_to_start_at_a_time",
                        Integer.toString(METACHANNELS_TO_START_AT_A_TIME)));
        configlogger.info("Starting metachannels after "
        ...
        for (int threadNum = 0; threadNum < command_threads.length; threadNum++) {
            ...
            if (context == null) {
                ...
                logger.error("JCA Context not initialized for thread" + threadNum
                        + ". If you see this, we should a sleep() ahead of this message.");
            } else {
    ...
    private void startMiscTasksScheduler(final ConfigService configService) {
        ...
        long tomorrowAfternoon =
                ((currentEpochSeconds / (PartitionGranularity.PARTITION_DAY.getApproxSecondsPerChunk())) + 1)
        ...
        miscTasksScheduler.scheduleAtFixedRate(
                new MetadataUpdater(),
                tomorrowAfternoon - currentEpochSeconds,
                PartitionGranularity.PARTITION_DAY.getApproxSecondsPerChunk(),
                TimeUnit.SECONDS);
    }
    ...
    public int assignJCACommandThread(String pvName, String iocHostName) {
        ...
        if (channel != null) {
            ...
            if (pvName.equals(pvNameOnly)) {
                logger.debug(
                        "We seem to have a channel already for " + pvName + ". Returning its JCA Command thread id.");
            }
    ...
    public boolean doesContextMatchThread(Context context, int jcaCommandThreadId) {
        ...
        if (contextForThreadId != null) {
            ...
            // We should never get here; but in the case we do failing in this assertion is less harmful than spewing
            // the message with logs...
            return true;
        }
    }
    ...
    public ScheduledThreadPoolExecutor getScheduler() {
        if (scheduler == null)
            scheduler = (ScheduledThreadPoolExecutor) Executors.newScheduledThreadPool(1, new ThreadFactory() {
                @Override
                public Thread newThread(Runnable r) {
            ...
            if (pubSubEvent.getType().equals("ComputeMetaInfo")) {
                ...
                try {
                    ...
                    if (fieldName != null && !fieldName.isEmpty()) {
                        logger.debug("We are not asking for extra fields for a field value " + fieldName + " for pv "
                                + pvName);
                    ...
                    JSONDecoder<UserSpecifiedSamplingParams> decoder =
                    ...
                    PubSubEvent confirmationEvent = new PubSubEvent(
                            "MetaInfoRequested", pubSubEvent.getSource() + "_" + ConfigService.WAR_FILE.MGMT, pvName);
                ...
                try {
                    ...
                    PubSubEvent confirmationEvent = new PubSubEvent(
                            "StartedArchivingPV", pubSubEvent.getSource() + "_" + ConfigService.WAR_FILE.MGMT, pvName);
                    configService.getEventBus().post(confirmationEvent);
                } catch (Exception ex) {
                ...
                try {
                    ...
                    // PubSubEvent confirmationEvent = new PubSubEvent("MetaInfoAborted", pubSubEvent.getSource() + "_"
    ...
    /**
     * @param newDisconnectCheckTimeoutSeconds This is to be used only for unit testing purposes...
    ...
    private void startArchivingPV(String pvName) throws Exception {
        ...
        logger.info("Archiving PV " + pvName + "using " + samplingMethod.toString() + " with a sampling period of "
                + samplingPeriod + "(s)");
    ...
    private final class DisconnectChecker implements Runnable {
        ...
        private boolean checkChannelDisconnectTime(
                LinkedList<String> disconnectedPVNames,
                LinkedList<String> needToStartMetaChannelPVNames,
                long disconnectTimeoutInSeconds,
