...
public class JCACommandThread extends Thread {
        ...
        try {
            if (jca == null) {
                ...
                for (ContextExceptionListener exl : ex_lsnrs)
                    if (exl != log_handler) jca_context.removeContextExceptionListener(exl);
                ...
                for (ContextMessageListener cml : msg_lsnrs)
                    if (cml != log_handler) jca_context.removeContextMessageListener(cml);
            }

        } catch (Exception e) {
            //
            logger.error("exception when initing Context in JCACommandThread", e);
        }
    }
    ...
    private Runnable getCommand() {
        synchronized (command_queue) {
            if (command_queue.size() > 0) return command_queue.removeFirst();
        }
        return null;
    }
