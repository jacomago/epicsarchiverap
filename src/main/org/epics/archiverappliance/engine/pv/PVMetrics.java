...
public class PVMetrics {
    ...
    public void addTimestampWrongEventCount(Instant incorrectTimeStamp) {
        if (lastEventFromIOCTimeStamp != null
                ...
                && incorrectTimeStamp.equals(lastEventFromIOCTimeStamp)) {
            return;
        }
    ...
    public double getStorageRate() {
        ...
        if (lastStartEpochSeconds == 0) {
            storageRateTemp =
                    (double) this.storageSize / (System.currentTimeMillis() / 1000 - this.initialEpochSeconds);
        } else {
    ...
    public LinkedList<Map<String, String>> getDetailedStatus() {
        ...
        addDetailedStatus(
                statuses,
                "Archiver DBR type (initial)",
                this.archDBRTypes == null ? "Unkown" : this.archDBRTypes.toString());
        addDetailedStatus(
                statuses,
                "Archiver DBR type (from CA)",
                this.newCADBRType == null ? "Unkown" : this.newCADBRType.toString());
        ...
        addDetailedStatus(
                statuses, "The SCAN period (ms) after applying the jitter factor", "" + this.scanPeriodMillis);
        addDetailedStatus(statuses, "Is this PV currently connected?", this.isConnected ? "yes" : "no");
        addDetailedStatus(
                statuses,
                "Connection state at last connection changed event",
                String.valueOf(this.lastConnectionEventState));
        addDetailedStatus(
                statuses,
                "When did we receive the last event?",
                TimeUtils.convertToHumanReadableString(this.secondsOfLastEvent));
        addDetailedStatus(
                statuses,
                "What did we last push the data to the short term store?",
                TimeUtils.convertToHumanReadableString(this.lastRotateLogsEpochSeconds));
        addDetailedStatus(
                statuses,
                "When did we request CA to make a connection to this PV?",
                TimeUtils.convertToHumanReadableString(connectionRequestMadeEpochSeconds));
        addDetailedStatus(
                statuses,
                "When did we first establish a connection to this PV?",
                TimeUtils.convertToHumanReadableString(connectionFirstEstablishedEpochSeconds));
        addDetailedStatus(
                statuses,
                "When did we last lose and reestablish a connection to this PV?",
                TimeUtils.convertToHumanReadableString(connectionLastRestablishedEpochSeconds));
        addDetailedStatus(
                statuses,
                "When did we last lose a connection to this PV?",
                TimeUtils.convertToHumanReadableString(this.connectionLastLostEpochSeconds));
        addDetailedStatus(
                statuses,
                "How many times have we lost and regained the connection to this PV?",
                Long.toString(connectionLossRegainCount));
        ...
        addDetailedStatus(
                statuses,
                "How many events lost because the timestamp is in the far future or past so far?",
                Long.toString(this.timestampWrongEventCount));
        addDetailedStatus(
                statuses,
                "Instant of last event from the IOC - correct or not.",
                this.getLastEventFromIOCTimeStampStr());
        addDetailedStatus(
                statuses,
                "How many events lost because the sample buffer is full so far?",
                Long.toString(this.sampleBufferFullLostEventCount));
        addDetailedStatus(
                statuses,
                "How many events lost because the DBR_Type of the PV has changed from what it used to be?",
                Long.toString(this.invalidTypeLostEventCount));
        addDetailedStatus(
                statuses,
                "How many events lost totally so far?",
                Long.toString(this.timestampWrongEventCount
                        + this.sampleBufferFullLostEventCount
                        + this.invalidTypeLostEventCount));
        if (storageSize > 0 && eventCounts > 0) {
            addDetailedStatus(
                    statuses,
                    "Average bytes per event",
                    twoSignificantDigits.format(((double) this.storageSize) / this.eventCounts));
        }
        double estimatedEventRate = this.getEventRate();
        addDetailedStatus(
                statuses,
                "Estimated event rate (events/sec)",
                (estimatedEventRate <= 0.0) ? "Not enough info" : twoSignificantDigits.format(estimatedEventRate));
        ...
        addDetailedStatus(
                ...
                (estimatedStorageRateInBytesPerSec <= 0.0)
                        ? "Not enough info"
                        : twoSignificantDigits.format(estimatedStorageRateInKiloBytesPerHour));
        ...
        addDetailedStatus(
                ...
                (estimatedStorageRateInBytesPerSec <= 0.0)
                        ? "Not enough info"
                        : twoSignificantDigits.format(estimatedStorageRateInMegaBytesPerDay));
        double estimatedStorageRateInGigaBytesPerYear =
                this.getStorageRate() * 60 * 60 * 24 * 365 / (1024 * 1024 * 1024);
        addDetailedStatus(
                statuses,
                "Estimated storage rate (GB/year)",
                (estimatedStorageRateInBytesPerSec <= 0.0)
                        ? "Not enough info"
                        : twoSignificantDigits.format(estimatedStorageRateInGigaBytesPerYear));
        return statuses;
    }
    ...
    public static final String CONNECTION_LOST_SECONDS_KEY = "cnxlostepsecs";
    public static final String CONNECTION_REGAINED_SECONDS_KEY = "cnxregainedepsecs";
    /**
    ...
    public void addConnectionLostRegainedFields(DBRTimeEvent event) {
        ...
        logger.debug(() -> MessageFormat.format(
                "Adding cnxlostepsecs {0} and cnxregainedepsecs for pv {1} at {2} onto event @ {3}",
                connectionLastLostEpochSeconds, pvName, cnxregainedsecs, event.getEpochSeconds()));
        ...
        if (firstDataAfterStartUp) {
            ...
            event.addFieldValue(CONNECTION_LOST_SECONDS_KEY, Long.toString(connectionLastLostEpochSeconds));
            event.addFieldValue(CONNECTION_REGAINED_SECONDS_KEY, Long.toString(cnxregainedsecs));
            ...
            if (isFirstDataAfterConnection) {
                if (connectionLastLostEpochSeconds != 0 && connectionLastLostEpochSeconds != cnxregainedsecs) {
                    if (logger.isDebugEnabled()) {
                        logger.debug(() ->
                                "Adding cnxlostepsecs and cnxregainedepsecs after regaining connection for pv " + pvName
                                        + " at " + cnxregainedsecs + " onto event @ " + event.getEpochSeconds());
                    }
                    event.addFieldValue(CONNECTION_LOST_SECONDS_KEY, Long.toString(connectionLastLostEpochSeconds));
                    event.addFieldValue(CONNECTION_REGAINED_SECONDS_KEY, Long.toString(cnxregainedsecs));
                    connectionLastLostEpochSeconds = 0;
                    ...
                    logger.debug(() ->
                            "Skipping adding cnxlostepsecs and cnxregainedepsecs after regaining connection for pv "
                                    + pvName);
                }
                isFirstDataAfterConnection = false;
            }
