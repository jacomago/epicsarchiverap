...
public class RemoveAliasAction implements BPLAction {
    ...
    private static Logger logger = LogManager.getLogger(
        RemoveAliasAction.class.getName()
    );
    ...
    public void execute(
        ...
        if (typeInfo == null) {
            logger.error(
                "We do not yet support removing aliases for PVs which have not completed their workflow " +
                    pvName
            );
        ...
        if (infoForPV.equals(configService.getMyApplianceInfo())) {
            ...
            infoValues.put(
                "desc",
                "Removed an alias " + aliasName + " for PV " + pvName
            );
            ...
            logger.info(
                "Redirecting removeAlias request for PV to " +
