...
public class ReshardPV implements BPLAction {
    ...
    public ReshardPV(ConfigService configService) {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (srcPVName == null || srcPVName.isEmpty()) {
        ...
        if (srcApplianceInfo.getIdentity().equals(destApplianceInfo.getIdentity())) {
            logger.error("Attempting to reshard onto same appliance - " + srcApplianceInfo.getIdentity() + "/"
        ...
        if (!destApplianceIdentity.equals(configService.getMyApplianceInfo().getIdentity())) {
            ...
            logger.info(
                    "Redirecting resharding request for PV to " + destApplianceIdentity + " using URL " + redirectURL);
        ...
        while (configService.getApplianceForPV(destPVName) != null) {
            ...
            if (nameTries++ > 100) {
                try (PrintWriter out = resp.getWriter()) {
                    String errorMsg =
                            "Unable to get a temporary name even after 100 tries. Giving up on resharding " + srcPVName;
                    logger.error(errorMsg);
                    infoValues.put("validation", errorMsg);
                    out.println(JSONValue.toJSONString(infoValues));
                    return;
        ...
        try (BasicContext context = new BasicContext()) {
            ...
            try (PrintWriter out = resp.getWriter()) {
                String errorMsg =
                        "Exception appending data for " + destPVName + ". Giving up on resharding " + srcPVName;
        ...
        if (beforeEventCount != afterEventCount) {
            try (PrintWriter out = resp.getWriter()) {
                String errorMsg = "When transferring data for pv " + srcPVName
                        + " we are not transferring the correct number of events. The source has "
                        + beforeEventCount
                        + " and the dest has "
                        + afterEventCount
        ...
        String deleteURL = srcApplianceInfo.getMgmtURL() + "/deletePV?pv=" + URLEncoder.encode(srcPVName, "UTF-8")
                + "&deleteData=true";
