...
public class RenamePVAction implements BPLAction {
    private static final Logger logger = LogManager.getLogger(RenamePVAction.class.getName());
    ...
    public void execute(HttpServletRequest req, HttpServletResponse resp, ConfigService configService)
        ...
        if (currentPVName == null || currentPVName.isEmpty()) {
            resp.sendError(HttpServletResponse.SC_BAD_REQUEST);
            return;
        }

        String newPVName = req.getParameter("newname");
        if (newPVName == null || newPVName.isEmpty()) {
            resp.sendError(HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        ...
        if (!info.getIdentity().equals(configService.getMyApplianceInfo().getIdentity())) {
            String redirectURL =
                    info.getMgmtURL() + "/renamePV?pv=" + URLEncoder.encode(currentPVName, StandardCharsets.UTF_8)
                            + "&newname=" + URLEncoder.encode(newPVName, StandardCharsets.UTF_8);
            logger.info(
                    () -> "Redirecting rename request for PV to " + info.getIdentity() + " using URL " + redirectURL);
