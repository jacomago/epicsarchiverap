...
public class SkipAliasCheckAction implements BPLAction {
    ...
    private static Logger logger = LogManager.getLogger(
        SkipAliasCheckAction.class.getName()
    );
    ...
    public void execute(
        ...
        if (typeInfo != null) {
            logger.error(
                "When skipping the alias check for " +
                    pvName +
                    ", the PV already has a typeinfo."
            );
            resp.sendError(HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        ...
        if (configService.getMgmtRuntimeState().isPVInWorkflow(pvName)) {
            UserSpecifiedSamplingParams params =
                configService.getUserSpecifiedSamplingParams(pvName);
            if (params == null) {
                logger.error(
                    "When skipping the alias check for " +
                        pvName +
            ...
            if (req.getParameter("doNotProxy") != null) {
                ...
                ProxyUtils.routeURLToOtherAppliances(
                    configService,
