...
public class PutPVTypeInfo implements BPLAction {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse resp, ConfigService configService)
        ...
        TypeInfoAndJsonObject updatedTypeInfo = this.readInNewTypeInfo(
                ...
                (typeInfo == null
                        ? true
                        : false)); // If we are creating a new typeInfo, we should automatically pause it.
        ...
        if (!pvName.equals(pvNameFromTypeInfo)) {
            String msg = "The PV name in the call " + pvName + " does not match the PV name in the typeinfo "
                    + pvNameFromTypeInfo;
            logger.error(msg);
        ...
        if (typeInfo == null) {
            ...
            if (!applianceIdentity.equals(applianceIdentityFromUpdatedTypeInfo)) {
                String msg = "The appliance identity for an existing PVTypeInfo " + applianceIdentity
                        + " does not match the appliance identity in the typeinfo "
                        + applianceIdentityFromUpdatedTypeInfo + " for pv " + pvName;
                logger.error(msg);
                resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, msg);
                return;
            }
    ...
    private TypeInfoAndJsonObject readInNewTypeInfo(
            HttpServletRequest req, HttpServletResponse resp, boolean pauseGeneratedTypeInfo) throws IOException {
