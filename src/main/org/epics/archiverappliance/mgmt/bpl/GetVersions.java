...
public class GetVersions implements BPLAction {
    ...
    public void execute(
        ...
        if (id == null || id.equals("")) {
            ...
            logger.debug(
                "No id specified, returning the id of this appliance " +
                    applianceInfo.getIdentity()
            );
        } else {
            logger.debug(
                "Getting Appliance info for appliance with identity " + id
            );
        ...
        try (PrintWriter out = resp.getWriter()) {
            ...
            Path versionPath = Paths.get(
                req.getServletContext().getRealPath("ui/comm/version.txt")
            );
            String mgmt_version = Files.readAllLines(
                versionPath,
                Charset.defaultCharset()
            ).get(0);
            versions.put("mgmt_version", mgmt_version);
            getVersion(
                applianceInfo.getEngineURL(),
                "engine",
                versions,
                mgmt_version
            );
            getVersion(
                applianceInfo.getEtlURL(),
                "etl",
                versions,
                mgmt_version
            );
            getVersion(
                applianceInfo.getRetrievalURL(),
                "retrieval",
                versions,
                mgmt_version
            );
            ...
            logger.error(
                "Exception marshalling typeinfo for appliance " + id,
                ex
            );
    ...
    private void getVersion(
        ...
        JSONObject componentVersion = GetUrlContent.getURLContentAsJSONObject(
            finalURL
        );
