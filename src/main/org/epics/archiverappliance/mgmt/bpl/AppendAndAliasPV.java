...
public class AppendAndAliasPV implements BPLAction {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse resp, ConfigService configService)
        ...
        if (olderPVName == null || olderPVName.isEmpty()) {
            resp.sendError(HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        String newerPVName = req.getParameter("newerpv");
        if (newerPVName == null || newerPVName.isEmpty()) {
            resp.sendError(HttpServletResponse.SC_BAD_REQUEST);
            return;
        }
        ...
        if (olderTypeInfo.getDBRType() != newerTypeInfo.getDBRType()) {
            infoValues.put(
                    "validation",
                    "The PVs " + olderPVName + " and " + newerPVName
                            + " are of different DBR Types. Cannot consolidate.");
            logger.error(infoValues.get("validation"));
            try (PrintWriter out = resp.getWriter()) {
                out.println(JSONValue.toJSONString(infoValues));
            }
        ...
        if (!newerApplianceInfo
            ...
            logger.info("Redirecting appendAndAliasPV request for older PV " + olderPVName + " and newer PV "
                    + newerPVName + " using URL " + redirectURL);
        ...
        if (consolidateStatus == null
                || consolidateStatus.equals("")
                || !consolidateStatus.containsKey("status")
                || !((String) consolidateStatus.get("status")).equalsIgnoreCase("ok")) {
            try (PrintWriter out = resp.getWriter()) {
        ...
        while (configService.getApplianceForPV(destPVName) != null) {
            ...
            if (nameTries++ > 100) {
                try (PrintWriter out = resp.getWriter()) {
                    String errorMsg = "Unable to get a temporary name even after 100 tries. Giving up on appending "
                            + newerPVName;
                    logger.error(errorMsg);
                    infoValues.put("validation", errorMsg);
                    out.println(JSONValue.toJSONString(infoValues));
        ...
        long distantFutureEpochSeconds = System.currentTimeMillis() / 1000
        ...
        if (epochForFirstEvent == distantFutureEpochSeconds) {
            String errorMsg =
                    "We dont seem to have any data for " + newerPVName + ". This operation is equivalent to a rename";
            logger.warn(errorMsg);
        } else {
            logger.info("Copying over data from older PV " + olderPVName + " upto "
                    + TimeUtils.convertToHumanReadableString(epochForFirstEvent));
        }
...

        {
            ...
            try (BasicContext context = new BasicContext()) {
                ...
                if (callables != null) {
                    for (Callable<EventStream> callable : callables) {
                        try (EventStream st = callable.call()) {
                            logger.info(
                                    "Appending data from " + st.getDescription().getSource());
                            destPlugin.appendData(context, destPVName, st);
                        }
                    }
                } else {
                    try (PrintWriter out = resp.getWriter()) {
                        String errorMsg = "There seems to be no data for " + olderPVName
                ...
                try (PrintWriter out = resp.getWriter()) {
                    String errorMsg = "Exception appending data for " + destPVName + ". Giving up on append and alias  "
                            + olderPVName;
        ...
        if (beforeEventCountTotal - afterEventCountDest > 10) {
            ...
            try (PrintWriter out = resp.getWriter()) {
                String errorMsg = "When copying data into a temporary PV, we seem to be missing  "
        ...
        {
            String deleteURL = newerApplianceInfo.getMgmtURL() + "/deletePV?pv="
                    + URLEncoder.encode(newerPVName, "UTF-8") + "&deleteData=true";
            ...
            if (deleteStatus == null
                    || deleteStatus.equals("")
                    || !deleteStatus.containsKey("status")
                    || !((String) deleteStatus.get("status")).equalsIgnoreCase("ok")) {
            ...
            if (renameStatus == null
                    || renameStatus.equals("")
                    || !renameStatus.containsKey("status")
                    || !((String) renameStatus.get("status")).equalsIgnoreCase("ok")) {
                logger.error("Invalid status renaming temporary PV using " + renameURL);
                try (PrintWriter out = resp.getWriter()) {
                    infoValues.put("status", "Rename failed");
                    infoValues.put(
                            "desc", "Could not rename temporary PV " + destPVName + " to source PV " + newerPVName);
                    out.println(JSONValue.toJSONString(infoValues));
                    return;
                }
            } else {
        ...
        {
            String deleteURL = olderApplianceInfo.getMgmtURL() + "/deletePV?pv="
                    + URLEncoder.encode(olderPVName, "UTF-8") + "&deleteData=true";
        ...
        {
            String addAliasURL = newerApplianceInfo.getMgmtURL() + "/addAlias?pv="
                    + URLEncoder.encode(newerPVName, "UTF-8") + "&aliasname=" + URLEncoder.encode(olderPVName, "UTF-8");
            ...
            if (allAliasStatus == null
                    || allAliasStatus.equals("")
                    || !allAliasStatus.containsKey("status")
                    || !((String) allAliasStatus.get("status")).equalsIgnoreCase("ok")) {
