...
public class ResumeArchivingPV implements BPLAction {
    ...
    private void resumeSinglePV(HttpServletRequest req, HttpServletResponse resp, ConfigService configService)
        ...
        if (info == null) {
            ...
            logger.debug(
                    "Asking engine to start archiving PV. This should create a channel object if it does not exist");
            String engineResumeURL =
                    info.getEngineURL() + "/resumeArchivingPV" + "?pv=" + URLEncoder.encode(pvName, "UTF-8");
    ...
    private void resumeMultiplePVs(HttpServletRequest req, HttpServletResponse resp, ConfigService configService)
        ...
        List<HashMap<String, String>> response =
                BulkPauseResumeUtils.pauseResumeByAppliance(pvNames, configService, askingToPausePV);
