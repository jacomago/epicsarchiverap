 ...
 *
 */
public class ImportConfig implements BPLAction<ConfigService> {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse resp, ConfigService configService)
        ...
        try (InputStream is = new BufferedInputStream(req.getInputStream())) {
            ...
            for (Object configObj : unmarshalledConfiguration) {
                ...
                if (applianceInfo == null) {
                    logger.error("Unable to determine appliance information for appliance " + applianceIdentity
                            + " when importing configuration for " + pvName);
                    continue;
                }
        ...
        for (String applianceIdentity : pvsForAppliances.keySet()) {
            ...
            if (pvsForAppliance.size() > 1) {
                ...
                JSONObject response = GetUrlContent.postDataAndGetContentAsJSONObject(
                        importConfigURL, GetUrlContent.from(pvsForAppliance));
