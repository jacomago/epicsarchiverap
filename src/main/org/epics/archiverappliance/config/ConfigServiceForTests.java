...
public class ConfigServiceForTests extends DefaultConfigService {
    ...
    /**
     * All unit test PV names are expected to begin with this. This name is supposed to be something that we will not
    ...
    public ConfigServiceForTests(File webInfClassesFolder, int jcaCommandThreadCount) throws ConfigException {
        ...
        try {
            ...
            Path configPath = Paths.get(this.webInfClassesFolder.getAbsolutePath() + File.separatorChar + DEFAULT_ARCHAPPL_PROPERTIES_FILENAME);
            configPath = configPath.normalize();
            String msg = String.format("Could not find config file:%s", configPath);
    ...
    public static String getDefaultPBTestFolder() {
        ...
        logger.info("mts folder is " + defaultFolder);
