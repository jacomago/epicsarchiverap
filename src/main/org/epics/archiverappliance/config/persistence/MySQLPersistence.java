...
public class MySQLPersistence implements ConfigPersistence {
    ...
    public MySQLPersistence() throws ConfigException {
        try {
            ...
            try (Connection conn = theDataSource.getConnection()) {
                ...
                configlogger.info(
                        String.format("RDB Driver is '%s' '%s'", meta.getDriverName(), meta.getDriverVersion()));

                if (Pattern.compile(".*sqlite.*", Pattern.CASE_INSENSITIVE)
                        .matcher(name)
                        .matches()) {
        ...
        try {
            ...
            try (Connection conn = theDataSource.getConnection()) {
                try (PreparedStatement stmt = conn.prepareStatement(
                        "SELECT typeInfoJSON AS typeInfoJSON FROM PVTypeInfo WHERE JSON_EXTRACT(typeInfoJSON, '$.applianceIdentity') = ?;")) {
            ...
            ForkJoinPool customThreadPool =
                    new ForkJoinPool(Math.max(Runtime.getRuntime().availableProcessors() / 2, 1));
            List<PVTypeInfo> typeInfos = customThreadPool
                    .submit(() -> typeInfoStrs.parallelStream()
    ...
    public PVTypeInfo getTypeInfo(String pvName) throws IOException {
        return getValueForKey(
                "SELECT typeInfoJSON AS typeInfoJSON FROM PVTypeInfo WHERE pvName = ?;",
                pvName,
                new PVTypeInfo(),
                PVTypeInfo.class,
                "getTypeInfo");
    ...
    public UserSpecifiedSamplingParams getArchivePVRequest(String pvName) throws IOException {
        return getValueForKey(
                "SELECT userParams AS userParams FROM ArchivePVRequests WHERE pvName = ?;",
                pvName,
                new UserSpecifiedSamplingParams(),
                UserSpecifiedSamplingParams.class,
                "getArchivePVRequest");
    }
    ...
    public void putArchivePVRequest(String pvName, UserSpecifiedSamplingParams userParams) throws IOException {
        ...
        switch (dialect) {
            ...
            case MySQL:
                sql =
                        "INSERT INTO ArchivePVRequests (pvName, userParams) VALUES (?, ?) ON DUPLICATE KEY UPDATE userParams = ?;";
                break;
            case SQLite:
                sql =
                        "INSERT INTO ArchivePVRequests (pvName, userParams) VALUES (?, ?) ON CONFLICT(pvName) DO UPDATE SET userParams = ?;";
                break;
        }
    ...
    public List<String> getExternalDataServersKeys() throws IOException {
        return getKeys(
                "SELECT serverid AS serverid FROM ExternalDataServers ORDER BY serverid;",
                "getExternalDataServersKeys");
    }

    @Override
    public String getExternalDataServer(String serverId) throws IOException {
        return getStringValueForKey(
                "SELECT serverinfo AS serverinfo FROM ExternalDataServers WHERE serverid = ?;",
                serverId,
    ...
    public void putExternalDataServer(String serverId, String serverInfo) throws IOException {
        ...
        switch (dialect) {
            ...
            case MySQL:
                sql =
                        "INSERT INTO ExternalDataServers (serverid, serverinfo) VALUES (?, ?) ON DUPLICATE KEY UPDATE serverinfo = ?;";
                break;
            case SQLite:
                sql =
                        "INSERT INTO ExternalDataServers (serverid, serverinfo) VALUES (?, ?) ON CONFLICT(serverid) DO UPDATE SET serverinfo = ?;";
                break;
        }
    ...
    public String getAliasNamesToRealName(String pvName) throws IOException {
        return getStringValueForKey(
                "SELECT realName AS realName FROM PVAliases WHERE pvName = ?;", pvName, "getAliasNamesToRealName");
    }
    ...
    public List<String> getAliasNamesForRealName(String realName) throws IOException {
        ...
        return getStringListValueForKey(
                "SELECT pvName FROM PVAliases WHERE realName=?;", realName, "getAliasNamesForRealName");
    }
