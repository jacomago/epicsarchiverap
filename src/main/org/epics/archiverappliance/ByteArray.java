...
public class ByteArray {
    ...
    private static final int MAX_BUFFER_SIZE =
            (LineByteStream.MAX_LINE_SIZE * LineByteStream.MAX_ITERATIONS_TO_DETERMINE_LINE) + 1024;
    /**
    ...
    public void doubleBufferSize() throws LineTooLongException {
        ...
        if (newSize > MAX_BUFFER_SIZE)
            throw new LineTooLongException(
                    "Current size is " + data.length + ". Doubling it will cause us to reach over the limit.");
            ...
            if (b == LineEscaper.ESCAPE_CHAR) {
                ...
                switch (b) {
                    case LineEscaper.ESCAPE_ESCAPE_CHAR:
                        unescapedData[unescapedLen++] = LineEscaper.ESCAPE_CHAR;
                        break;
                    case LineEscaper.NEWLINE_ESCAPE_CHAR:
                        unescapedData[unescapedLen++] = LineEscaper.NEWLINE_CHAR;
                        break;
                    case LineEscaper.CARRIAGERETURN_ESCAPE_CHAR:
                        unescapedData[unescapedLen++] = LineEscaper.CARRIAGERETURN_CHAR;
                        break;
                    default:
