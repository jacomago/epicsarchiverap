...
public class SearchInPBFileTest {
    ...
    public void testSeekToTime() throws Exception {
        PlainPBStoragePlugin pbplugin = new PlainPBStoragePlugin(FileExtension.PB);
        pbSetup.setUpRootFolder(pbplugin, FileExtension.PB);
		short year = TimeUtils.getCurrentYear();
	    Instant start = TimeUtils.getStartOfYear(year);
        long numberOfSamples =
                GenerateData.generateSineForPV("Sine1", 0, ArchDBRTypes.DBR_SCALAR_DOUBLE, FileExtension.PB, start, start.plusSeconds(10000));
        try {
