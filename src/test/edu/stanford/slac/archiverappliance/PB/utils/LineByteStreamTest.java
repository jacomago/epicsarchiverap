...
public class LineByteStreamTest {
    ...
    private static void seekandCheck(File f, long position, String expectedNumberStr) throws IOException {
        try (LineByteStream lis = new LineByteStream(f.toPath(), position)) {
            ...
            Assertions.assertEquals(expectedNumberStr, gotNumberStr);
        }
    }
    ...
    public void testLineByteStream() throws Exception {
        ...
        try (LineByteStream lis = new LineByteStream(f.toPath())) {
            ...
            try {
                ...
                logger.error("Exception around " + expectedNumber + t.getMessage(), t);
                Assertions.fail();
            }
            ...
            Assertions.assertEquals(
                    EvenNumberSampleFileGenerator.MAXSAMPLEINT,
                    lastNumber,
    ...
    public void testSmallFileSeekToLastLine() throws Exception {
        ...
        try (LineByteStream lis = new LineByteStream(f.toPath())) {
            ...
            byte[] line = lis.readLine();
            int lastNumber = Integer.parseInt(new String(line, StandardCharsets.UTF_8));
            Assertions.assertEquals(8, lastNumber, "Testing last line number Got " + lastNumber);
            assert f.delete();
        }
    }
    ...
    private void testEndPositionForStartEnd(File f, int start, int end) throws Exception {
        try (LineByteStream lis = new LineByteStream(f.toPath(), 7L * start, 7L * end)) {
            ...
            while (line != null) {
                ...
                Assertions.assertEquals(gotNum, expectedNum, "Expected " + expectedNum + " obtained " + gotNum);
    ...
    public void testLargeLinesSeekToLastLine() throws Exception {
        for (int linesize = 3; linesize < LineByteStream.MAX_LINE_SIZE * 3; linesize++) {
            String fileName =
                    ConfigServiceForTests.getDefaultPBTestFolder() + "/" + "LargeLineByteStreamSeekToLast.txt";
            File f = new File(fileName);
            assert !f.exists() || f.delete();
            ...
            try {
                while (line != null) {
                    if (line.length > 0) {
                        int gotNumber = Integer.parseInt(new String(line, StandardCharsets.UTF_8));
                        if (gotNumber != expectedNumber) {
                            Assertions.fail("Expected " + expectedNumber + " got " + gotNumber + " in count "
                                    + linecount + " lastReadPointer " + lis.lastReadPointer + " currentReadPosition "
                                    + lis.currentReadPosition);
                        }
                    } else {
    ...
    public void testLastAndFirstLinesWithBoundedStream() throws Exception {
        ...
        String fileName =
                ConfigServiceForTests.getDefaultPBTestFolder() + "/" + "LastAndFirstLinesWithBoundedStream.txt";
        File f = new File(fileName);
        assert !f.exists() || f.delete();
                ...
                try (LineByteStream lis = new LineByteStream(f.toPath(), start, end)) {
                    ...
                    while (line != null) {
                        ...
                        Assertions.assertEquals(
                                17,
                                line.length,
                                "Length should be 17; instead it is " + line.length + " for start=" + start
                                        + " and end=" + end);
                        Assertions.assertEquals(
                                val,
                                formatter.format(expectedLineNumber),
        ...
        // Keep the end fixed and vary the start
        {
            ...
            for (long start = 1; start < fileSize - 10; start += 1) {
                try (LineByteStream lis = new LineByteStream(f.toPath(), start, fileSize)) {
                    int linenumber = 1;
                    lis.seekToFirstNewLine();
                    byte[] line = lis.readLine();
