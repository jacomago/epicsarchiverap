        ...
        for (ArchDBRTypes dbrType : ArchDBRTypes.values()) {
            ...
            for (int severity = 0; severity < 4; severity++) {
                for (int status = 0; status < 22; status++) {
                    try {
                        DBR dbr = getJCASampleValue(dbrType, status * severity, severity, status);
                        DBRTimeEvent e = DefaultEvent.fromDBR(dbrType, dbr, Map.of(), false);
                        assertEquals(
        ...
        switch (type) {
            ...
            case DBR_WAVEFORM_SHORT:
                ...
                retvsh = new DBR_TIME_Short(ArrayUtils.toPrimitive(
                        Collections.nCopies(1, (short) value).toArray(new Short[0])));
            ...
            case DBR_WAVEFORM_FLOAT:
                ...
                retvf = new DBR_TIME_Float(
                        ArrayUtils.toPrimitive(Collections.nCopies(value, (float) Math.cos(value * Math.PI / 3600))
                                .toArray(new Float[0])));
            ...
            case DBR_WAVEFORM_ENUM:
                ...
                retven = new DBR_TIME_Enum(ArrayUtils.toPrimitive(
                        Collections.nCopies(1024, (short) value).toArray(new Short[0])));
            ...
            case DBR_WAVEFORM_BYTE:
                ...
                retvb = new DBR_TIME_Byte(
                        ArrayUtils.toPrimitive(Collections.nCopies(65536 * value, ((byte) (value % 255)))
                                .toArray(new Byte[0])));
            ...
            case DBR_WAVEFORM_INT:
                ...
                retvint = new DBR_TIME_Int(ArrayUtils.toPrimitive(
                        Collections.nCopies(value, value * value).toArray(new Integer[0])));
            ...
            case DBR_WAVEFORM_DOUBLE:
                ...
                // Varying number of copies of a typical value
                retvd = new DBR_TIME_Double(
                        ArrayUtils.toPrimitive(Collections.nCopies(value, Math.sin(value * Math.PI / 3600))
