        ...
        for (ArchDBRTypes dbrType : ArchDBRTypes.values()) {
            ...
            try (SimulationEventStream simstream = new SimulationEventStream(
                ...
                for (Event ev : simstream) {
                    ...
                    DBRTimeEvent pb0 = DefaultEvent.fromDBREvent((DBRTimeEvent) ev);
                    String fieldName = "HIHI";
                    String fieldValue = "0.0";
                    pb0 = pb0.cloneWithExtraFieldValues(Map.of(fieldName, fieldValue), false);
                    ByteArray raw1 = pb0.getRawForm();

                    DBRTimeEvent pb1DbrTimeEvent = DefaultEvent.fromByteArray(dbrType, year, raw1);
                    assertTrue(
                    ...
                    pb1DbrTimeEvent = pb1DbrTimeEvent.cloneWithExtraFieldValues(Map.of(), true);
                    assertTrue(
                            "Adding 1 field value after marking as actual change isActualChange is false ",
                            pb1DbrTimeEvent.isActualChange());

                    // Test adding multiple fields at the same time.
                    DBRTimeEvent pbm0 = DefaultEvent.fromDBREvent((DBRTimeEvent) ev);
                    Map<String, String> values = new HashMap<String, String>();
                    values.put("HIHI", "0.0");
                    values.put("LOLO", "-1.0");
                    values.put("LOPR", "1000.0");
                    values.put("HOPR", "-10000.0");
                    pbm0 = pbm0.cloneWithExtraFieldValues(values, false);
