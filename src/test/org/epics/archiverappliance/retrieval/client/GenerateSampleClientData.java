...
public class GenerateSampleClientData {
    private static final Logger logger = LogManager.getLogger(GenerateSampleClientData.class.getName());
    /**
     * @param args
     */
    public static void main(String[] args) throws Exception {
        if (args == null || args.length != 1) {
            System.err.println(
                    "Usage: java org.epics.archiverappliance.retrieval.client.GenerateSampleClientData <Folder>");
            return;
        }
    ...
    private static void generateOneFileWithWellKnownPoints(File destFolder) throws Exception {
        ...
        try (FileOutputStream fos = new FileOutputStream(destFile)) {
            ...
            for (int day = 0; day < 366; day++) {
                POJOEvent event =
                        new POJOEvent(ArchDBRTypes.DBR_SCALAR_DOUBLE, ts, new ScalarValue<Double>((double) day), 0, 0);
                ByteArray val = event.getRawForm();
                fos.write(val.data, val.off, val.len);
                fos.write(LineEscaper.NEWLINE_CHAR);
                ts = Instant.ofEpochMilli(
                        ts.toEpochMilli() + PartitionGranularity.PARTITION_DAY.getApproxSecondsPerChunk() * 1000);
            }
        }
    }

    private static void writeHeader(String pvName, ArchDBRTypes dbrType, short year, FileOutputStream fos)
            throws Exception {
            ...
            for (int day = 0; day < 366; ) {
                ...
                for (int c = 0; c < chunkSize; c++) {
                    ...
                    POJOEvent event = new POJOEvent(
                            ArchDBRTypes.DBR_SCALAR_DOUBLE, ts, new ScalarValue<Double>((double) day++), 0, 0);
                    ByteArray val = event.getRawForm();
                    fos.write(val.data, val.off, val.len);
                    fos.write(LineEscaper.NEWLINE_CHAR);
                    ts = Instant.ofEpochMilli(
    ...
    private static void generateOneDaysWorthOfDBRDoubleData(File destFolder) throws Exception {
        ...
        try (FileOutputStream fos = new FileOutputStream(destFile)) {
            ...
            for (int seconds = 0; seconds < PartitionGranularity.PARTITION_DAY.getApproxSecondsPerChunk(); seconds++) {
                POJOEvent event = new POJOEvent(
                        ArchDBRTypes.DBR_SCALAR_DOUBLE, ts, new ScalarValue<Double>((double) seconds), 0, 0);
                ByteArray val = event.getRawForm();
                fos.write(val.data, val.off, val.len);
                fos.write(LineEscaper.NEWLINE_CHAR);
