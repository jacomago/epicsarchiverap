...
import org.epics.archiverappliance.common.*;
...
public class DataReductionPostProcessorsTest {
    ...
    private void cleanDataFolders(String shortTermFolderName, String mediumTermFolderName, String longTermFolderName) throws IOException {
    ...
    public void testPostProcessor(String reduceDataUsing, FileExtension ltsFileExtension) throws Exception {
        ...
        final String rawPVName =
                ConfigServiceForTests.ARCH_UNIT_TEST_PVNAME_PREFIX + DataReductionPostProcessorsTest.class.getSimpleName() + reduceDataUsing + ltsFileExtension.getSuffix();
        final String reducedPVName = rawPVName + "reduced";

        String shortTermFolderName = ConfigServiceForTests.getDefaultShortTermFolder() + String.format("/%s/shortTerm", reduceDataUsing);
        String mediumTermFolderName = ConfigServiceForTests.getDefaultPBTestFolder() + String.format("/%s/mediumTerm", reduceDataUsing);
        String longTermFolderName = ConfigServiceForTests.getDefaultPBTestFolder() + String.format("/%s/longTerm", reduceDataUsing);
